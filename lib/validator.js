...
async function validateCSS(filePath, options = {}) {
  ...
  const result = await stylelint.lint({
    files: filePath,
    config: config,
    report: options.format === 'json' ? 'json' : 'string'
  });

  const output = result.results[0];

  if (options.format === 'json') {
    // Only stringify the results array to avoid deprecated 'output' property
    console.log(JSON.stringify({ results: result.results }, null, 2));
