...
function extractRules(configContent) {
  try {
    // Calibre's config is a JavaScript module with a config object
    // We need to extract the rules from it
    const rulesMatch = configContent.match(/rules:\s*{([\s\S]*?)^\s*}/m);
    if (rulesMatch) {
      // Parse the rules object safely
      const rulesStr = `{${rulesMatch[1]}}`;
      // Use Function constructor for safer parsing than eval
...
async function validateCSS(filePath, options = {}) {
  ...
  if (options.format === 'json') {
    ...
    // Default text format
    if (output && output.errored) {
      console.log(output.output);
    }
  }

  return {
    totalErrors: output ? output.errored ? 1 : 0 : 0,
    totalWarnings: 0,
