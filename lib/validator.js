...
async function validateCSS(filePath, options = {}) {
  ...
  const errorCount = output && output.warnings ? output.warnings.filter(w => w.severity === 'error').length : 0;
  const warningCount = output && output.warnings ? output.warnings.filter(w => w.severity === 'warning').length : 0;
